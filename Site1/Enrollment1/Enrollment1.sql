...
CREATE OR REPLACE PACKAGE BODY enr1_p AS
	...
	PROCEDURE Enrl1
	IS
	stid Enrollment1.SID%TYPE;
	crsid Enrollment1.CID%TYPE;
	pcname Course2.PreCName%TYPE;
	cidd Course2.CID%TYPE;
	total1 int;
	total2 int;
	total3 int;
	n number;
	...
	BEGIN
		...
		END LOOP;
		--course exists/not
		Select COUNT(*) into total1 FROM Course2 where Course2.CID= crsid;
		Select COUNT(*) into total3 FROM Student1 where Student1.SID= stid;
		...
		IF (total1>0 and total3>0) THEN 
			...
			Select SUM(Course2.Credit) into tocre FROM Course2 Inner JOIN
			Enrollment1 ON Course2.CID = Enrollment1.CID Where (Enrollment1.SID = stid);
			DBMS_OUTPUT.PUT_LINE(tocre);
			IF tocre<15 THEN 
				SELECT PreCName into pcname from Course2 where CID = crsid;
				...
				IF (pcname!=' ') THEN
					...
					SELECT COUNT(*) into total2 from Enrollment1 WHERE (CID= cidd and SID =stid);
					--if yes then 
					DBMS_OUTPUT.PUT_LINE(total2);
					IF total2>0 THEN
						INSERT into Enrollment1 VALUES((n+1),crsid,stid);
